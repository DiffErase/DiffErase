...
class AudioDiffusion:
    ...
    def generate_spectrogram_and_audio_from_audio(
    ...
    ) -> Union[Tuple[Image.Image, Tuple[int, np.ndarray]], Tuple[Image.Image, Tuple[int, np.ndarray], torch.Tensor]]:
        ...
        Returns:
            PIL Image: mel spectrogram
            (float, np.ndarray): sample rate and raw audio
            If return_mel_tensor=True, also returns torch.Tensor: mel spectrogram in log scale, shape (n_mels, time)
        """
        ...
        result = self.pipe(
            ...
            encoding=encoding,
            return_dict=False,
            return_mel_tensor=return_mel_tensor,
        )
        
        if return_mel_tensor:
            images, (sample_rate, audios), mel_tensor = result
            return images[0], (sample_rate, audios[0]), mel_tensor[0]  # Return first item from batch
        else:
            images, (sample_rate, audios) = result
            return images[0], (sample_rate, audios[0])
